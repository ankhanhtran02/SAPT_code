...
class ModelArguments:
    ...
    model_name_or_path: str = field(
        default="Salesforce/codet5-small",metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"}
    )
...
def main():
    ...
    logger.warning(
        ...
        + f"distributed training: {bool(training_args.local_rank > 0)}, 16-bits training: {training_args.fp16}"
    )
    ...
    if training_args.do_predict:
        ...
        if not prompt_config["run_single"]:
            ...
            with open(os.path.join(save_path, "attention_weights.pkl"), 'wb') as f:
                ...
                print(np.array(trainer.model.encoder.all_attn_weights).mean(axis=0))
                pickle.dump(np.array(trainer.model.encoder.all_attn_weights).mean(axis=0), f)
