...
class T5LayerNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.weight.dtype in [torch.float16, 
        # torch.bfloat16
        ]:
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value, key_attention_weights, lora_layer=None):
            ...
            if key_value_states is None:
                ...
                if lora_layer is not None:
                    if key_attention_weights is not None:
                        lora_states = agg_lora_states(hidden_states, lora_layer, self.previous_lora_weights_v, key_attention_weights)
                    else:
                ...
                if lora_layer is not None:
                    if key_attention_weights is not None:
                        lora_states = agg_lora_states(key_value_states, lora_layer, self.previous_lora_weights_v, key_attention_weights)
                    else:
        ...
        # get query states
        if key_attention_weights is not None:
            query_states = shape(self.q(hidden_states)+agg_lora_states(hidden_states, self.lora_q, self.previous_lora_weights_q, key_attention_weights))
        else:
        ...
        value_states = project(
            hidden_states, self.v, key_value_states, past_key_value[1] if past_key_value is not None else None, key_attention_weights, self.lora_v
        )
    ...
    def __init__(self, config, embed_tokens=None, prompt_config=None):
        ...
        if not self.is_decoder and not prompt_config["run_single"]:
            ...
            self.trans_input = nn.Sequential(
                ...
                nn.Linear(100, config.d_model, bias=False),
                nn.SiLU(),
    ...
    def forward(
        ...
        if not self.is_decoder and not self.prompt_config["run_single"]:
            ...
            if self.previous_prompts_keys is not None:
                ...
                key_attention_weights = self.cal_attention(past_prompt_key, inputs_embeds)

                if self.is_inference:
                    self.all_attn_weights.append(key_attention_weights.squeeze().mean(dim=0).detach().to(torch.float).cpu().numpy())
            else:
                key_attention_weights = self.cal_attention(prompt_key.repeat(batch_size, 1, 1), inputs_embeds)
